...
class TritonPythonModel:
    ...
    def initialize(self, args):
        ...
        if args["model_name"] == "resnet50":
            self.model = resnet50(pretrained=False, progress=True)
            self.model.eval()
            # pdb.set_trace()
            self.model.load_state_dict(flow.load(path.join(args["model_repository"], "1", "model")))
            self.model.to(flow.device("cuda:1"))
        elif args["model_name"] == "sd15":
            ...
            raise ValueError(f"get unsupport model name is {args['model_name']}")
